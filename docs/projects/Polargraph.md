...
{.newthought}A few years ago{/.newthought}, back when GPT-3.5 was first released, and I still spent most of my days sitting in a high school classroom, I had a project idea. Perhaps one of my best named project ideas to date: The *Generative Pen-trained Transformer* or GPenT for short.

GPenT was awesome, I was going to build out a color-changing pen plotter with some funky coreXY-esque kinematic, and alongside it, try my hand at writing one of those [SVG Blob Generators](https://www.blobmaker.app/) to generate a bunch of shapes to draw.<label for="sn-blobs" class="margin-toggle sidenote-number"></label><input type="checkbox" id="sn-blobs" class="margin-toggle"/><span class="sidenote"><a href="https://www.blobmaker.app/">blobmaker.app</a> - a simple blob generator. Here are a few random ones:<svg class="blob-sidenote-svg" viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;margin:0.4em 0 0;display:block;"></svg></span> These 'blob' generators are simple enough, they take in some input variables from a user in the form of sliders and output a downloadable SVG, yet I wanted to try my hand at something new. Instead of a UX with sliders, I would pass in a raw string of numbers into this generator, numbers generated by GPT-3.5.
...
The operating principle of the polargraph is simple enough. Take some rectangular work area with a motor in each of the top corners, string a belt over each of those motors, append a counterweight on one side of each belt, and attach the other side of both belts to a single gondola. This gondola will thus be moved when either motor rotates. This gives us a *bipolar* coordinate system: the pen is located not by an $(x, y)$ position, as on a typical cartesian plotter, but by the two belt lengths $L_1$ and $L_2$. Each motor acts as a pole, and turning just one of them sweeps the pen along an arc about the other.

<div class="polargraph-diagram">
<svg viewBox="0 0 1000 700" xmlns="http://www.w3.org/2000/svg">
    <defs>
        <clipPath id="pgWorkplane"><rect x="100" y="120" width="800" height="500"/></clipPath>
    </defs>
    <g id="pgGrid" class="pg-grid" clip-path="url(#pgWorkplane)"></g>
    ...
    <text class="pg-pole-label" x="116" y="156" text-anchor="middle" font-size="17">pole</text>
    <text class="pg-pole-label" x="884" y="156" text-anchor="middle" font-size="17">pole</text>
...
{.marginnote}Where I intend to hang the polargraph in my living room.{/.marginnote}
...
{.newthought}The gondola{/.newthought} is the real heart of this beast, so we'll take on its assembly first. Our gondola here is a modified version of a Makelangelo gondola remix I found on [printables](https://www.printables.com/model/116596-makelangelo-5-wall-hanging-plotter-gondola-with-ge).{.sidenote}[Makelangelo 5 gondola remix on Printables](https://www.printables.com/model/116596-makelangelo-5-wall-hanging-plotter-gondola-with-ge){/.sidenote}
...
{.marginnote}Tungsten rounds positioned in the gondola body{/.marginnote}
...
{.marginnote}Lumber haul for the polargraph frame{/.marginnote}
...
{.marginnote}Pizza-fueled build session{/.marginnote}
...
{.marginnote}Cutting dados with Graham{/.marginnote}
...
<img src="../../assets/images/Polargraph/clamps.JPG" alt="">
<img src="../../assets/images/Polargraph/woodFillTopside.JPG" alt="">
</div>
...
<div class="figure-grid grid-2x1">
<img src="../../assets/images/Polargraph/stepperJigs.JPG" alt="">
<img src="../../assets/images/Polargraph/stepperHole.JPG" alt="">
</div>
...
{.marginnote}Timing pulleys mounted on both steppers{/.marginnote}
...
<div class="figure-grid grid-2x1">
<img src="../../assets/images/Polargraph/mountedStep.JPG" alt="">
<img src="../../assets/images/Polargraph/mountedSwitch.JPG" alt="">
</div>
...
{.marginnote}Cable passthrough hole near the stepper mount{/.marginnote}
...
<div class="figure-grid grid-2x1">
<img src="../../assets/images/Polargraph/finalWires.JPG" alt="">
<img src="../../assets/images/Polargraph/beltLegnth.JPEG" alt="">
</div>
...
<div class="figure-grid grid-2x1">
<img src="../../assets/images/Polargraph/C14wire.JPG" alt="">
<img src="../../assets/images/Polargraph/mountedC14.JPG" alt="">
</div>
...
{.marginnote}Mounting the french cleat - standing on a chair{/.marginnote}
...
{.marginnote}The mounted polargraph{/.marginnote}
...
{.newthought}I've always{/.newthought} really enjoyed pen plotter art, or frankly, any thin line drawings. One of my favorite twitter accounts is [@adamfuhrer](https://x.com/adamfuhrer?lang=en), who fills my timeline with a ton of aesthetic plots/sketches. One of his recent tweets really caught my eye, highlighting the works of Channa Horwitz:
...
Horwitz invented a complete visual notation system constrained to 1/8" graph paper several decades ago (1968), which she coined: Sonakinatography.{.sidenote}Channa Horwitz (1932–2013) developed Sonakinatography as a method of visualizing rhythm and movement on graph paper. Her works are held in the collections of LACMA, the Whitney, and the Centre Pompidou.{/.sidenote} All of her works were done entirely by hand, which is absolutely bonkers, as they're so meticulous they look computer generated. While I'm not quite confident I hold a steady enough hand to practice these plots true to her method, I now happen to own a wonderful, wall-mounted polargraph!
...
{.newthought}All these turtle scripts{/.newthought} and sonakinatography algorithms are cool, but this is the Generative Pen-trained Transformer after all! I certainly can't call this project complete without some ML shenanigans.
...
Nonetheless, the network in its current state is available on [HuggingFace](https://huggingface.co/twarner/dcode-sd-gcode) as well as baked into the plotter [dot] local interface so you can [try it out](https://huggingface.co/spaces/twarner/dcode) for yourself!{.sidenote}[dcode model on HuggingFace](https://huggingface.co/twarner/dcode-sd-gcode){/.sidenote}
...
{.newthought}I am so stoked{/.newthought} with how this project came together, and it feels really good to have dusted off an old project from the shelf and to have seen it through. My apartment walls are a little less bare now. I've captured some of my favorite plots, check them out: 
...
(function() {
    ...
    // --- bipolar coordinate overlay -------------------------------------
    const SVGNS = 'http://www.w3.org/2000/svg';
    const grid = document.getElementById('pgGrid');
    if (grid && !grid.childElementCount) {
        [[leftMotorX, leftMotorY], [rightMotorX, rightMotorY]].forEach(function(m) {
            for (let r = 70; r < 1200; r += 70) {
                const c = document.createElementNS(SVGNS, 'circle');
                c.setAttribute('cx', m[0]); c.setAttribute('cy', m[1]); c.setAttribute('r', r);
                grid.appendChild(c);
            }
        });
    }
    function makeArc() { const p = document.createElementNS(SVGNS, 'path'); p.setAttribute('class', 'pg-arc'); return p; }
    function makeLabel(sub) {
        const t = document.createElementNS(SVGNS, 'text');
        t.setAttribute('class', 'pg-radius-label');
        t.setAttribute('font-size', '26');
        t.setAttribute('text-anchor', 'middle');
        t.appendChild(document.createTextNode('L'));
        const ts = document.createElementNS(SVGNS, 'tspan');
        ts.setAttribute('font-size', '17'); ts.setAttribute('dy', '6'); ts.textContent = sub;
        t.appendChild(ts);
        return t;
    }
    const overlay = document.getElementById('pgOverlay');
    const labels = document.getElementById('pgLabels');
    const arcAboutRight = makeArc(), arcAboutLeft = makeArc();
    const labelL1 = makeLabel('1'), labelL2 = makeLabel('2');
    if (overlay && !overlay.childElementCount) { overlay.appendChild(arcAboutRight); overlay.appendChild(arcAboutLeft); }
    if (labels && !labels.childElementCount) { labels.appendChild(labelL1); labels.appendChild(labelL2); }

    function arcPath(cx, cy, px, py, halfDeg) {
        const r = Math.hypot(px - cx, py - cy);
        const base = Math.atan2(py - cy, px - cx);
        const h = halfDeg * Math.PI / 180, a0 = base - h, a1 = base + h;
    ...
    function placeLabel(el, mx, my, ax, ay) {
        const midx = (mx + ax) / 2, midy = (my + ay) / 2;
        const dx = ax - mx, dy = ay - my, L = Math.hypot(dx, dy) || 1;
        let nx = -dy / L, ny = dx / L;
        if (ny > 0) { nx = -nx; ny = -ny; }  // keep label on the upper side of the belt
        const off = 28;
        el.setAttribute('x', (midx + nx * off).toFixed(1));
        el.setAttribute('y', (midy + ny * off).toFixed(1));
    }
    ...
    function updateAnimation() {
        ...
        // bipolar overlay: tangent arcs through the pen + belt-length labels
        arcAboutRight.setAttribute('d', arcPath(rightMotorX, rightMotorY, gondolaX, gondolaY, 15));
        arcAboutLeft.setAttribute('d', arcPath(leftMotorX, leftMotorY, gondolaX, gondolaY, 15));
        placeLabel(labelL1, leftMotorX, leftMotorY, leftAttachX, leftAttachY);
        placeLabel(labelL2, rightMotorX, rightMotorY, rightAttachX, rightAttachY);
